...
double dust(Mat &src, int i, int j, Mat &map, double path);
...
//	trtst = ori;
	...
	//ori = trtst;
	for (i=ori.rows*2/3; i<ori.rows; i++){
...
void cleanNoise(Mat &src, int pathMax){
	...
	for(i=0; i<src.rows; i++){
		for (j=0; j<src.cols; j++){
			double path = dust(src, i, j, map, 0);
...
void getRoi(Mat &src, Mat &roi){
	...
	for (i=src.rows*2/3; i<src.rows-1; i++){
			...
			if(src.at<uchar>(i, j)==255){
				...
				if(foundRoi){
					...
					if(bTop==-1){
						...
						bTop = i;
						bBot = i;
					}else if (abs(roiIni-bRight)<30 && abs(roiEnd -bLeft)<30 && abs(i-bBot)<100){
